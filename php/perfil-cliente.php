...
if (!empty($_POST['documento']) && !empty($_POST['nombre']) && !empty($_POST['apellidos']) && !empty($_POST['telefono']) && !empty($_POST['correo']) && !empty($_POST['clave1']) && !empty($_POST['clave2'])) {
	if ($_POST['clave1']==$_POST['clave2']) {
		$actualizar= $conn->prepare('UPDATE tblcliente SET identificacion=:identificacion,nombres=:nombres,apellidos=:apellidos,telefono=:telefono,correo=:correo,contrasena=:contrasena WHERE identificacion=:identificacion');
...
if (!empty($_FILES['foto']) && !empty($_POST['identificacion'])) {
	$foto_a=$conn->prepare('UPDATE tblcliente SET identificacion=:identificacion,foto=:foto WHERE identificacion=:identificacion');
...
<form action="" method="post" enctype="multipart/form-data">
	<div class="row">
		<div class="col-3">
			<img src="<?php echo $user['foto']; ?>" width="200" height="200" class="rounded-circle bg-light border border-dark" name="foto" id="img_profile">
		</div>
		<div class="col-6">
			<div class="form-row">
			  	<div class="col-md-12 mb-3">
				    ...
				    <div class="input-group">
						...
						<input type="text" name="identificacion" value="<?php echo $user['identificacion']; ?>" class="d-none" readonly>
				     	<input type="file" id="files" class="form-control" name="foto">
					</div>
				</div>
			</div>
