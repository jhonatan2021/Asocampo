<?php
  ...
  if (isset($_SESSION['user_id'])) {
    $records = $conn->prepare('SELECT * FROM tbladmin WHERE identificacion = :id');
...
<body class="bg-light">
  ...
  <nav>
    <ul id="menu">
      <li><a href="">Menú</a>
        <ul>
          <li><a class="<?php echo $pagina == 'perfil-admin' ? 'active' : ''; ?>" href="?p=perfil-admin">Perfil</a>
          </li>
          <li><a class="<?php echo $pagina == 'gestor-producto' ? 'active' : ''; ?>" href="?p=gestor-producto">Productos</a></li>
        </ul>  
      </li>
    </ul>
  </nav>
  <div class="row bg-light">
    <div class="col-12">
      <?php 
        $pagina = isset($_GET['p']) ? strtolower($_GET['p']) : 'gestor-producto';
